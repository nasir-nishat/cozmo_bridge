...
// ─── REQUEST DETECTION ───────────────────────────────────────────────────────
export async function detectGuestRequest(message: string, senderName?: string): Promise<string | null> {
    const senderContext = senderName ? `Sender name: "${senderName}"\n` : '';
    const result = await callLLMWithFallback(
        COZMO_SYSTEM,
        `${senderContext}Message: "${message}"\n\nRespond in one line only. Follow the RULES and EXAMPLES exactly.`
    );
    if (!result || result.toLowerCase() === 'null') return null;
    return result.trim();
}

export async function detectGuestRequestWithHistory(history: string[], latestMessage: string): Promise<string | null> {
...
async function callOpenAI(system: string, user: string, maxTokens = 1000): Promise<string> {
...
export type SupportedLang = 'ZH-TW' | 'ZH-CN' | 'JA' | 'TH' | 'EN';
...
export async function detectGuestLanguage(guestName: string, phone: string): Promise<string> {
    try {
        const raw = await callOpenAI(
            LANG_SELECTOR_SYSTEM,
        ...
        if (lang.includes('simplified')) return 'ZH-CN';
        if (lang.includes('traditional')) return 'ZH-TW';
        return 'EN';
    } catch (e: any) {
...
export async function detectGroupRejection(message: string): Promise<boolean> {
    const result = await callLLM(
        `You are analyzing a WhatsApp message from a hotel guest.
...
export async function wasAlreadySent(groupKey: string, templateText: string, sinceMinutes = 120): Promise<boolean> {
    const { getRecentMessages } = await import('./messageBuffer');
    const messages = getRecentMessages(groupKey, sinceMinutes);
    if (messages.length === 0) return false;
    const history = messages.map(m => `[${m.sender}]: ${m.text}`).join('\n');
    const result = await callLLM(
        'You are checking if a scheduled message was already sent manually in a group chat. Reply with YES or NO only.',
        `Template message:\n"${templateText}"\n\nRecent group messages:\n${history}\n\nDoes any message match the template at 70% or more similarity? Reply YES or NO only.`,
        5
    ).catch(() => 'NO');
    return result.trim().toUpperCase().startsWith('YES');
}
