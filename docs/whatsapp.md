...
- `GROUP_CREATION_ENABLED=false` while running on a personal linked-device number. Re-enable only
  deliberately, after checking `src/data/pending-group-creation.json` and WA account health.
...
- QR re-link can wipe the instance webhook. The bridge now self-registers `EVOLUTION_WEBHOOK_URL`
  on startup and during the WA watchdog check with `MESSAGES_UPSERT`, `MESSAGES_UPDATE`,
  `SEND_MESSAGE`, `CONNECTION_UPDATE`, `GROUP_PARTICIPANTS_UPDATE`, `GROUPS_UPSERT`, and
  `CONTACTS_UPSERT`.
- Deduplication must not let `MESSAGES_UPDATE` or `SEND_MESSAGE` consume a message ID before
  `MESSAGES_UPSERT` is handled. Keep message handling dedupe scoped to `messages.upsert`.
