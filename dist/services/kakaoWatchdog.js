...
exports.recordKakaoHeartbeat = recordKakaoHeartbeat;
exports.getLastKakaoHeartbeat = getLastKakaoHeartbeat;
exports.checkKakaoHeartbeat = checkKakaoHeartbeat;
...
let lastAlertAt = 0;
function recordKakaoHeartbeat() {
    lastHeartbeat = Date.now();
}
function getLastKakaoHeartbeat() {
    return lastHeartbeat;
}
async function checkKakaoHeartbeat() {
    if (lastHeartbeat === 0)
        return; // no heartbeat ever received — skip until first ping
    const now = Date.now();
    ...
    await (0, notify_1.sendAlert)(`📵 <b>KakaoTalk Bot Unresponsive</b>\n─────────────────\n` +
