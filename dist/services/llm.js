...
async function callOpenAI(system, user, maxTokens = 1000) {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
        try {
            const res = await axios_1.default.post('https://api.openai.com/v1/chat/completions', payload, { headers });
...
async function translateMessage(text, targetLang) {
    ...
    const result = await callOpenAI(TRANSLATION_SYSTEM, `Translate the following message to ${langLabel[targetLang]}:\n\n${text}`, 16000);
...
async function detectLanguage(text) {
    const result = await callOpenAI('You are a language detector. Respond with ONLY one of these codes: ZH-TW, ZH-CN, JA, TH, EN, KO, OTHER. No explanation.', `Detect the language of this text: "${text.slice(0, 200)}"`);
