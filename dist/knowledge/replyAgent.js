...
let _qaExamples = null;
function loadQAExamples() {
    if (_qaExamples)
        return _qaExamples;
    try {
        const p = path_1.default.resolve(__dirname, '..', '..', 'src', 'knowledge', 'qa-examples.json');
        const raw = JSON.parse(fs_1.default.readFileSync(p, 'utf-8'));
        _qaExamples = (raw.examples || []);
    }
    catch {
        _qaExamples = [];
    }
...
function pickQAExamples(query, category, limit = 3) {
...
function scoreEntry(entry, query, terms, propertyCode) {
...
function searchRelevantEntries(query, entries, propertyCode) {
...
function buildFactsBlock(routerResult, ctx) {
    ...
    for (const entry of allEntries) {
        ...
        if (entry.links.length)
            lines.push(`Link: ${entry.links[0]}`);
        lines.push('');
    }
    return lines.join('\n').trim();
}
function buildHistoryBlock(ctx) {
...
function buildQAExamplesBlock(routerResult, ctx) {
...
function buildUserPrompt(routerResult, ctx) {
...
`.trim();
function buildSystem(vibeGuide) {
...
async function generateReply(routerResult, knowledgeContext, vibeGuide) {
    ...
    if (!knowledgeContext.chatHistory.length)
        return 'ESCALATE';
    // No KB entries at all means nothing reliable to draw from.
    if (!knowledgeContext.entries.length && !routerResult.matchedEntries.length)
        return 'ESCALATE';
    ...
    if (!raw)
        return 'ESCALATE';
    if (raw.trim().toUpperCase() === 'ESCALATE')
        return 'ESCALATE';
    return raw;
}
